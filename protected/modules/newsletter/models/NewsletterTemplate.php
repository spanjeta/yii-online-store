...
class NewsletterTemplate extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('title, code, message, variable_info', 'required'),
			array('title, variable_info', 'length', 'max'=>255),
			array('code', 'length', 'max'=>30),
			array('status', 'length', 'max'=>1),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, code, message, variable_info, status', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('variable_info',$this->variable_info,true);
