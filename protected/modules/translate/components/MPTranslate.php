...
class MPTranslate extends CApplicationComponent {
	...
	function missingTranslation($event) {
		...
		$model = MessageSource::model ()->find ( 'LOWER(message)=:message AND LOWER(category)=:category', $attributes );
	 ...
	 *        	label of the link
	 * @param string $type
	 *        	accepted types are : link and button
	 * @return string
	 */
	...
	function translateDialogLink($label = 'Translate', $title = null, $type = 'link') {
		return $this->ajaxDialog ( $label, 'translate/translate/index', $title, $type, array (
				'data' => array (
						self::ID . '-missing' => self::$messages 
				) 
		) );
	}
	/**
	 ...
	 * @param string $label        	
	 * @param string $type
	 *        	accepted types are button and link
	 * @return string
	 */
	...
	private function ajaxDialog($label, $url, $title = null, $type = 'link', $ajaxOptions = array()) {
		...
		$content .= Yii::app ()->getController ()->widget ( 'zii.widgets.jui.CJuiDialog', array (
				'options' => array_merge ( $this->dialogOptions, array (
						'title' => $title 
				) ),
				'id' => $id 
		), true );
	 ...
	 *
	 * @param
	 *        	string | null $language language to set
	...
	function dropdown() {
        ...
        ' );
		return CHtml::dropDownList ( self::ID, $this->getLanguage (), $this->acceptedLanguages, array (
				'id' => self::ID 
		) );
	}
	/**
	 ...
	 *        	language to translate the message to, if null it will use the current language in use
	 * @param mixed $sourceLanguage
	 *        	language that the message is written in, if null it will use the application source language
	...
	function getGoogleAcceptedLanguages($targetLanguage = null) {
		...
		if (! isset ( $this->_cache [$cacheKey] )) {
			if (($cache = Yii::app ()->getCache ()) === null || ($languages = $cache->get ( $cacheKey )) === false) {
				$queryLanguages = $this->queryGoogle ( $targetLanguage !== null ? array (
						'target' => $targetLanguage 
				) : array (), 'languages' );
	 ...
	 * @param array $args        	
	 * @param string $method
	 *        	the method to use, use null to translate
	...
	protected function queryGoogle($args = array(), $method = null) {
		...
		if ($method !== null)
			$method = "/{$method}";
		$url = preg_replace ( '/%5B\d+%5D/', '', "https://www.googleapis.com/language/translate/v2{$method}?" . http_build_query ( array_merge ( $args, array (
