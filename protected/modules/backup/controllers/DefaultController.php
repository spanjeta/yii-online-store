...
class DefaultController extends Controller {
	...
	public function getData($tableName) {
		...
		foreach ( $dataReader as $data ) {
			...
			if ($this->fp) {fwrite ( $this->fp, $data_string );}
		}
	...
	public function actionCreate($data=true) {
		set_time_limit(0);
	...
	public function actionIndex() {
		...
		$list_zip = $this->getFileList ( '*.zip' );
		$list_files = array_merge ( $list_zip, $list );
		if ($list_files) {
			$list = array_map ( 'basename', $list_files );
			sort ( $list );
			foreach ( $list as $id => $filename ) {
				...
				if (date ( 'M-d-Y' . ' \a\t ' . ' g:i A', filemtime ( $path . $filename ) ) > date ( 'M-d-Y' . ' \a\t ' . ' g:i A', filectime ( $path . $filename ) )) {
					...
					$columns ['modified_time'] = 'Never Modified';
				}
	...
	public function actionRestore($file = null) {
		...
		if (! empty ( $files )) {
			...
			$last_file = $files [0];
			$sqlZipFile = $this->path . basename ( $last_file );
			$sqlFile = $this->unzip ( $sqlZipFile );
