...
/**
 * Html extends CHtml and provides additional features.
 *
 * @author Rodrigo Coelho <rodrigo@giix.org>
 */
class Html extends CHtml {
	 ...
	 * Controller::getRelatedData with the appropriate 'uncheckValue'.
	 ...
	 * <ul>
	 * <li>This method supports {@link ActiveRecord::representingColumn} and {@link ActiveRecord::toString}.</li>
	 ...
	 * Optional. If not specified, the {@link ActiveRecord::__toString} method will be used.
	...
	public static function listDataEx($models, $valueField = null, $textField = null, $groupField = '') {
		...
		if ($groupField === '') {
			foreach ($models as $model) {
				...
				if ($valueField === null)
					$value = ActiveRecord::extractPkValue($model, true);
				else
	...
	public static function selectData($value) {
		...
		if ((is_object($value) && is_subclass_of($value, 'ActiveRecord')) ||
				(is_array($value) && !empty($value) && is_object($value[0]) && is_subclass_of($value[0], 'ActiveRecord')))
			return ActiveRecord::extractPkValue($value, true);
		else
			return $value;
	}
	 ...
	 * Optional. If not specified, the {@link ActiveRecord::__toString} method will be used.
	 * In this case, the fist parameter ($model) can not be an array, it must be an instance of ActiveRecord.
	...
	public static function value($model, $attribute = null, $defaultValue = null)
	{
		if ($attribute === null) {
			if (is_object($model) && is_subclass_of($model, 'ActiveRecord'))
				return $model->__toString();
			...
			$ret =  $model->getValue($model, $attribute, $defaultValue);
