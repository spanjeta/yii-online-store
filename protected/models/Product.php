...
class Product extends BaseProduct {
	...
	public function getAverageRating($product_id) {
		...
		$getReviews= Rating::model ()->findByAttributes ( [
				'product_id' => $product_id
		] );
		
		
		/* $getReviews = Rating::find ()->where ( [
		 'product_id' => $product_id
		 ] )->all (); */
	...
	public function getProductRating($data)
	{
		...
		if ($data->getAverageRating ( $data->id )) {
			
			$averageStars = $data->getAverageRating ( $data->id );
	...
	public function getProductName($id = null) {
		$name = array();
		if($id != null){
			$models = Category::model()->findAllByAttributes(array(
					'id' => $id
			));
		}
		if($models != null){
	...
	protected function beforeDelete() {
		...
		if ($prodimages) {
			foreach ( $prodimages as $prodimage ) {
				$imagefile = Yii::app ()->basePath . '/..' . UPLOAD_PATH_USER . $prodimage->image_path;
			...
			if (! empty ( $this->category_id ))
				...
				if (! empty ( $this->color_id ))
					...
					if (! empty ( $this->store_id ))
						...
						public static function getAllBrandProducts($id) {
							$prod = [ ];
							$products = Product::model ()->findAllByAttributes ( array (
									'brand_id' => $id
							) );
